...
@router.get("/calendar/monthly", summary="查詢指定月份的所有提醒", response_model=List[NotificationResponse])
async def get_monthly_reminders(
    ...
    results = query.order_by(
        Notification.reminder_date_start.asc(), 
        Notification.reminder_time.asc()
    ).all()
...
async def update_reminder(
    ...
    db_reminder = db.query(Notification).filter(
...
@router.delete("/{reminder_id}", summary="刪除提醒", status_code=status.HTTP_204_NO_CONTENT)
