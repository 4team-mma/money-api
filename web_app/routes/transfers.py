...
async def update_transfer(
    ...
    old_from_acc = (
        db.query(Account)
        .filter(
            Account.account_id == old_tx.from_account_id,
            Account.user_id == current_user.user_id,
        )
        .one()
    )
    old_to_acc = (
        db.query(Account)
        .filter(
            Account.account_id == old_tx.to_account_id,
            Account.user_id == current_user.user_id,
        )
        .one()
    )
    ...
    new_from_id = (
        ...
        else old_tx.from_account_id
    )
    new_to_id = (
        data.to_account_id if data.to_account_id is not None else old_tx.to_account_id
    )
    ...
    new_from_acc = (
        db.query(Account)
        .filter(
            Account.account_id == new_from_id, Account.user_id == current_user.user_id
        )
        .one()
    )
    new_to_acc = (
        db.query(Account)
        .filter(
            Account.account_id == new_to_id, Account.user_id == current_user.user_id
        )
        .one()
    )
