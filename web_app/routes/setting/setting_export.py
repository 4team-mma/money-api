...
def create_pie_chart(income, expense, font_name):
    ...
    if data[0] == 0 and data[1] == 0: data[0] = 0.1
    pc.data = data
...
def generate_pdf_response(data, display_title, period_text, summary, user_nickname):
    ...
    doc = SimpleDocTemplate(output, pagesize=A4, rightMargin=45, leftMargin=45, topMargin=40, bottomMargin=50)
    elements = []
    
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle(name='Title_CH', fontName=font_name, fontSize=26, alignment=1, spaceAfter=5)
    # 右上角動態日期樣式
    date_style = ParagraphStyle(name='Date_CH', fontName=font_name, fontSize=14, alignment=2, textColor=colors.HexColor("#ef4444"))
    # 緊湊統計摘要
    summary_style = ParagraphStyle(name='Summary_CH', fontName=font_name, fontSize=11, leading=14, spaceBefore=0)
    footer_style = ParagraphStyle(name='Footer_CH', fontName=font_name, fontSize=9, alignment=1, textColor=colors.grey, spaceBefore=40)
    ...
    for item in data:
        table_data.append([str(item["日期"]), item["類型"], format_currency(item["金額"]), item["分類"], item["成員"], item["備註"]])
...
async def export_report(
    ...
    if time_range == "current-month":
        ...
        filters = [extract('year', AddRecord.add_date) == now.year, extract('month', AddRecord.add_date) == now.month]
    
    elif time_range == "last-month":
        last_m = now.replace(day=1) - pd.Timedelta(days=1)
        period_text, display_title = last_m.strftime("%Y年%m月"), "月度"
        filters = [extract('year', AddRecord.add_date) == last_m.year, extract('month', AddRecord.add_date) == last_m.month]
        ...
        filters = [extract('year', AddRecord.add_date) == target_year, extract('month', AddRecord.add_date).between(start_m, start_m + 2)]
    # --- B. 執行查詢 ---
    ...
    data_list = [{"日期": r.add_date, "類型": "收入" if r.add_type else "支出", "金額": float(r.add_amount), 
                  "分類": r.add_class, "成員": r.add_member, "備註": r.add_note or "-"} for r in records]
    ...
    if report_format == "pdf":
        ...
        return StreamingResponse(pdf_content, media_type="application/pdf", headers={"Content-Disposition": f"attachment; filename*=UTF-8''{encoded_filename}.pdf"})
    else:
        ...
        output.seek(0)
        return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", headers={"Content-Disposition": f"attachment; filename*=UTF-8''{encoded_filename}.xlsx"})