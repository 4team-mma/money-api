...
def get_dashboard_summary(user_id: int, db: Session = Depends(get_db)):
    ...
    # 包含：收支紀錄、轉帳轉出、轉帳轉入
    adds_ids = db.query(AddRecord.account_id.label("aid")).filter(
        AddRecord.user_id == user_id
    )
    trans_from_ids = db.query(Transaction.from_account_id.label("aid")).filter(
        Transaction.user_id == user_id
    )
    trans_to_ids = db.query(Transaction.to_account_id.label("aid")).filter(
        Transaction.user_id == user_id
    )
    ...
    usage_counts = (
        ...
        .group_by(all_usage.c.aid)
        .subquery()
    )

    # 步驟 C: Join 回 Account 表取得完整資訊
    most_frequent_data = (
        db.query(Account, usage_counts.c.cnt)
